...
class DocumentApiTest extends TestCase
{
    ...
    public function test_owner_can_update_document_metadata(): void
    {
        $user = User::factory()->create();
        $type = $this->createDocumentType();
        $newType = $this->createDocumentType(['name' => 'proposal']);
        $document = $this->createDocument($user, $type, 'Judul Lama');
        Sanctum::actingAs($user);
        ...
        $this->putJson("/api/documents/{$document->id}", [
            ...
            ->assertOk()
            ->assertJsonPath('message', 'Dokumen berhasil diperbarui.')
            ->assertJsonPath('data.title', 'Judul Baru')
            ->assertJsonPath('data.document_type.id', $newType->id)
            ->assertJsonPath('data.latest_version.version_number', 1);
    ...
    public function test_other_user_cannot_update_or_delete_document(): void
    {
        $owner = User::factory()->create();
        $otherUser = User::factory()->create();
        $document = $this->createDocument($owner, $this->createDocumentType(), 'Dokumen Privat');
        ...
        $this->putJson("/api/documents/{$document->id}", [
            'title' => 'Judul Disusupi',
        ])
            ->assertForbidden()
            ->assertJsonPath('message', 'Anda tidak memiliki akses untuk memperbarui dokumen ini.');

        $this->deleteJson("/api/documents/{$document->id}")
            ->assertForbidden()
            ->assertJsonPath('message', 'Anda tidak memiliki akses untuk menghapus dokumen ini.');
    ...
    public function test_owner_can_delete_document_and_its_private_files(): void
    {
        Storage::fake('local');
        $user = User::factory()->create();
        $document = $this->createDocument($user, $this->createDocumentType(), 'Dokumen Dihapus');
        Storage::disk('local')->put($document->latestVersion->file_path, 'PDF content');
        Sanctum::actingAs($user);
        ...
        Storage::disk('local')->assertMissing($document->latestVersion->file_path);
    }
