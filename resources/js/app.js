...
const createDocumentRow = (documentData) => {
    ...
    const detailLink = createTextElement('a', 'detail-link', 'Detail');
    const editBtn = createTextElement('button', 'library-action-btn library-edit-btn', 'Edit');
    const deleteBtn = createTextElement('button', 'library-action-btn library-delete-btn', 'Hapus');

    detailLink.href = `/documents/${documentData.id}`;
    editBtn.dataset.documentId = documentData.id;
    deleteBtn.dataset.documentId = documentData.id;
    actionGroup.className = 'library-actions';
    documentCell.append(title, topic);
    statusCell.appendChild(status);
    actionGroup.append(detailLink, editBtn, deleteBtn);
...
const findDocumentById = (id) => libraryDocuments.find((doc) => doc.id === Number(id));

const openEditDocumentModal = (documentData) => {
    document.getElementById('editDocumentTitle').value = documentData.title || '';
    document.getElementById('editDocumentTopic').value = documentData.topic || '';
    document.getElementById('editDocumentKeywords').value = documentData.keywords || '';
    document.getElementById('editDocumentDescription').value = documentData.description || '';
    document.getElementById('editDocumentForm').dataset.documentId = documentData.id;
...
const submitEditDocument = async (form, documentId, token) => {
    ...
    try {
        ...
        if (!response.ok) {
            showAlert(getErrorMessage(data, 'Dokumen gagal diperbarui.'), 'editDocumentAlert');
            return;
        }

        closeEditDocumentModal();
        showAlert(data.message || 'Dokumen berhasil diperbarui.', 'libraryAlert', 'success');
        renderDocumentLibrary(libraryDocuments);
    } catch {
        showAlert('Dokumen tidak dapat diperbarui. Silakan coba kembali.', 'editDocumentAlert');
    } finally {
        setSubmitting(form, false);
    }
};
...
const deleteDocument = async (documentId, token) => {
    ...
    const button = document.getElementById('confirmDeleteDocument');
    button.disabled = true;
    ...
    try {
        ...
        if (!response.ok) {
            showAlert(data.message || 'Dokumen gagal dihapus.', 'deleteDocumentAlert');
            return;
        }

        document.getElementById('deleteDocumentModal').classList.add('hidden');
        libraryDocuments = libraryDocuments.filter((doc) => doc.id !== Number(documentId));
        renderDocumentLibrary(libraryDocuments);
        showAlert(data.message || 'Dokumen berhasil dihapus.', 'libraryAlert', 'success');
    } catch {
        showAlert('Dokumen tidak dapat dihapus. Silakan coba kembali.', 'deleteDocumentAlert');
    } finally {
        button.disabled = false;
...
const loadDocumentLibrary = async (token) => {
    try {
        ...
        libraryDocuments = data.data || [];
        const initialFilters = new URLSearchParams(window.location.search);

        populateLibraryTypeFilter(libraryDocuments);
        ...
        ['documentSearch', 'documentTypeFilter', 'documentStatusFilter'].forEach((id) => {
            document.getElementById(id).addEventListener('input', () => renderDocumentLibrary(libraryDocuments));
        });
...
const initializeDocumentLibrary = () => {
    ...
    libraryToken = session.token;

    document.querySelector('[data-logout]')?.addEventListener('click', logout);

    document.getElementById('closeEditDocumentModal')?.addEventListener('click', closeEditDocumentModal);
    document.getElementById('cancelEditDocument')?.addEventListener('click', closeEditDocumentModal);
    document.getElementById('editDocumentForm')?.addEventListener('submit', (event) => {
        event.preventDefault();
    ...
    document.getElementById('cancelDeleteDocument')?.addEventListener('click', () => {
        document.getElementById('deleteDocumentModal').classList.add('hidden');
        hideAlert('deleteDocumentAlert');
    });
    document.getElementById('confirmDeleteDocument')?.addEventListener('click', () => {
        const documentId = document.getElementById('confirmDeleteDocument').dataset.documentId;
        if (documentId) deleteDocument(documentId, libraryToken);
    });

    document.getElementById('documentTableBody')?.addEventListener('click', (event) => {
        const target = event.target;
        const editBtn = target.closest('.library-edit-btn');
        const deleteBtn = target.closest('.library-delete-btn');

        if (editBtn) {
            const documentId = Number(editBtn.dataset.documentId);
            const documentData = findDocumentById(documentId);
            if (documentData) openEditDocumentModal(documentData);
            return;
        }

        if (deleteBtn) {
            const documentId = deleteBtn.dataset.documentId;
            document.getElementById('confirmDeleteDocument').dataset.documentId = documentId;
    ...
    loadDocumentLibrary(libraryToken);
};
