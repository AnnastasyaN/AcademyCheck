...
class JournalRecommendationController extends Controller
{
    ...
    public function index(Request $request, Document $document): JsonResponse
    {
        ...
        if (! $this->isArticle($document)) {
            return $this->validationError(null, 'Rekomendasi jurnal hanya tersedia untuk artikel ilmiah.');
        }
    ...
    public function generate(
        ...
        if (blank($articleText)) {
            return $this->validationError(null, 'Teks artikel belum tersedia untuk dianalisis.');
        }
        ...
        if ($journals->count() < 3) {
            return $this->validationError(null, 'Data jurnal eligible AI belum cukup. Lengkapi metadata minimal 3 jurnal dengan eligibility score 70+.');
        }
        ...
        try {
            ...
            return $this->badGateway('AI gagal membuat rekomendasi jurnal. Silakan coba kembali.');
        } catch (\RuntimeException $exception) {
            report($exception);

            return $this->badGateway('Format respons AI tidak valid.');
        } catch (Throwable $exception) {
            report($exception);

            return $this->serverError('Rekomendasi jurnal gagal karena kesalahan internal.');
        }

        return $this->created($saved, 'Rekomendasi jurnal berhasil dibuat.');
    }
